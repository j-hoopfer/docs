...
terraform {
  backend "s3" {
    bucket         = "mycompany-terraform-state-staging"  # Different bucket
    key            = "my-project/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
...
- [ ] AWS SSO profile configured (`mycompany-{env}`)
- [ ] CI/CD workflow updated with new account
- [ ] Account configuration created (`accounts/{env}/`)

**After Bootstrap:**

- [ ] S3 bucket `mycompany-terraform-state-{env}` exists
...
See [Phase 8: Next Steps](../steps/phase-8-next-steps.md) for enterprise-mycompany patterns.
