# Activity 3: Traffic Routing (Strangler Fig)

**Goal:** Configure ALB Listener Rules to selectively route a subset of traffic (by Header or Path) to the new Fargate service, leaving the bulk of traffic on EC2.

## Context & Themes

This connects the "backend" (ECS tasks) to the "frontend" (internet ALB). By using specific listener rules, we can safely expose the service without impacting existing traffic.

**Key Themes:**

- **Traffic Shaping:** Controlling the flow of requests.
- **Canary Release:** Testing with a small subset of traffic.
- **Risk Mitigation:** Avoiding a big bang cutover.

### Prerequisites

- [ ] [Service Deployment](2-service-deployment.md) completed (tasks running and healthy).
- [ ] ALB Listener ARN available.

## Feature 3: ALB Routing
...
### Story 3.1: Configure Host-Based Routing
