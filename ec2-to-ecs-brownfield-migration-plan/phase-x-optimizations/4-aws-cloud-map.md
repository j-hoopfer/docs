...
Implement AWS Cloud Map to enable internal DNS-based service discovery (e.g., `service-b.local`) for microservices communication without relying on internal load balancers.

### Context

Using Internal ALBs for every service adds latency and cost. Cloud Map provides a simpler, lower-cost alternative for services that can resolve DNS names directly.

> **Note:** In Phase 3, we set up **Internal DNS (Route 53) pointing to an Internal ALB**. This provided stable DNS names (e.g., `service.internal`) but traffic still flows through the ALB. Cloud Map removes the ALB entirely.

## Feature 4: Internal Service Communication
...
>
> During migration, you will use the **Internal ALB + Route 53 (Phase 3)** for all internal service-to-service communication. The Internal ALB is required for the Strangler Fig cutover pattern.
>
...
> **Migration phases:**
>
> 1. **During migration:** KrakenD and services → Internal DNS (`service.internal`) → Internal ALB → EC2/ECS
> 2. **Post-migration (optional):** Services → Cloud Map DNS → ECS directly (no ALB hop)

### Story 4.1: Enable AWS Cloud Map Service Discovery
...
### Story 4.2: Update Application Configurations for Internal Routing
