...
**Goal:** Create a centralized library of GitHub Actions workflows to standardize deployments across all 10 services, eliminating copy-paste errors and reducing maintenance overhead.

## Context & Themes

Before migrating the remaining services, we need to refactor that logic into reusable templates. This ensures that security scans, tagging strategies, and deployment steps are identical for every service.

**Key Themes:**

- **Standardization:** Consistent steps across services.
- **Maintainability:** Fix once, update everywhere.
- **Efficiency:** Reducing setup time for new services.

### Prerequisites

- [ ] Platform Repository Setup completed.
- [ ] CI/CD Pipeline knowledge from Phase 4.
...
- **Implementation Details:**
  - **Use Platform Repository:**
    - Repo: `infra-platform` (created in Phase 0)
    - This holds shared workflows (in `.github/workflows/`), in addition to platform Terraform.
  - **Reusable Workflow Template:**
    - File: `.github/workflows/ecs-deploy-template.yml` (inside `infra-platform`)
